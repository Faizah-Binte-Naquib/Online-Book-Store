...
if(isset($_POST["query"])){
 ...
 customer ON rents.Customer_ID = customer.Customer_ID where rents.Status = 0 
 ...
 OR rents.Rent_Return_Date LIKE '%".$search."%')";
}
else{
 $query = "
  ...
  if($months == 6 ){
    ...
    $output .= '
      ...
      <td><div id="'.$row['BookID'].'" class="booknameDesign viewBook"> ' . $row['BookName'] . '</div></td>
        ...
        <td><div style="text-align:center; background-color: rgb(54, 173, 54); color: white; padding: 5px;">'.$days.' days</div></td>
        <td>' . $row['Quantity'] . '</td>
        <td>'.$cost.'</td>
        <td>
        <a href="rentPendingBook.php?rentId='.$row['Rent_ID'].'" class="btn btn-outline-dark btn-rounded btn-sm">returned</a>
        </td>
      </tr>
    ';
    $i = $i + 1;
