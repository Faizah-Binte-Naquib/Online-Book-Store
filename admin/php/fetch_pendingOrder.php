...
if(isset($_POST["query"])){
 ...
 $query = " 
  ...
  WHERE buys.Status != 1 AND (books.BookName LIKE '%".$search."%' or customer.Customer_name LIKE '%".$search."%'
  or customer.Customer_address LIKE '%".$search."%' or buys.Order_Date LIKE '%".$search."%') ORDER BY buys.Order_ID DESC;
 ";
}
else{
 $query = "
 select * from buys,books,customer WHERE buys.BookID = books.BookID and buys.Customer_ID = customer.Customer_ID and buys.Status = 0 ORDER BY buys.Order_ID DESC";
}
   ...
   $price = $row['Quantity'] * $row['BookPrice'];
    $output .= '
      ...
      <td><button type="button" name="view" id="'.$row['Customer_ID'].'" class="btn btn-sm btn-link viewUser booknameDesign"> ' . $row['Customer_name'] . '</button></td> 
        ...
        <td>
        <a href="orderPending.php?id='.$row['Order_ID'].'" class="btn btn-outline-dark btn-rounded btn-sm">Delivered</a>
        </td>
      </tr>
    ';
    $i = $i + 1;
